...
class FaceWav2Vec2Model(Wav2Vec2Model):
    ...
    def forward(
        ...
        if dataset == "BIWI":
            # cut audio feature
            if extract_features.shape[1]%2 != 0:
                extract_features = extract_features[:, :-1]
            if frame_num and extract_features.shape[1]>frame_num*2:
                extract_features = extract_features[:, :frame_num*2]
        elif dataset == "vocaset":
            extract_features = linear_interpolation(extract_features, 50, 30,output_len=frame_num)
